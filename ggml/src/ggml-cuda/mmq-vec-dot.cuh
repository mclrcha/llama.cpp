...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q6_K_q8_1_mma(
        const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
#if defined(AMD_WMMA_AVAILABLE) && defined(RDNA4)
    // R9700: both 16 value sub-blocks of a q8_1 block share the activation scale.
    // Their integer dot products are scaled and summed in int32 (exact, |sum| < 2^24) before the single float conversion.
    ...
    // Super-block scale of each row of this lane.
    float dA[ntx][tile_C::ne];
#pragma unroll
    for (int n = 0; n < ntx; ++n) {
#pragma unroll
        for (int l = 0; l < tile_C::ne; ++l) {
            dA[n][l] = x_df[(i0 + n*tile_C::I + tile_C::get_i(l))*sram_stride];
        }
    }
...
#pragma unroll
        for (int n = 0; n < ntx; ++n) {
            load_ldmatrix(A0[n], x_qs + (i0 + n*tile_A::I)*sram_stride + k0,               sram_stride);
            load_ldmatrix(A1[n], x_qs + (i0 + n*tile_A::I)*sram_stride + k0 + tile_A::J, sram_stride);
        }

        // Scales of the two 16 value sub-blocks, sign extended by the byte loads.
        int sc0[ntx][tile_C::ne];
        int sc1[ntx][tile_C::ne];
#pragma unroll
        for (int n = 0; n < ntx; ++n) {
#pragma unroll
            for (int l = 0; l < tile_C::ne; ++l) {
                const int8_t * sc = (const int8_t *) (x_sc + (i0 + n*tile_C::I + tile_C::get_i(l))*sram_stride + k00/16);
                sc0[n][l] = sc[k01/4 + 0];
                sc1[n][l] = sc[k01/4 + 1];
            }
        }
...
#pragma unroll
        for (int j0 = 0; j0 < J; j0 += ntx*tile_C::J) {
            tile_B B0;
            tile_B B1;
            load_ldmatrix(B0, y_qs + j0*MMQ_TILE_Y_K + k01,             MMQ_TILE_Y_K);
            load_ldmatrix(B1, y_qs + j0*MMQ_TILE_Y_K + k01 + tile_B::J, MMQ_TILE_Y_K);
...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    const int c = __mul24(C0.x[l], sc0[n][l]) + __mul24(C1.x[l], sc1[n][l]);
                    sum[(j0/tile_C::J + n)*tile_C::ne + l] += float(c) * (dA[n][l]*dB);
                }
                // Keep the WMMAs of the next tile behind this point: few C tiles alive, no spills.
