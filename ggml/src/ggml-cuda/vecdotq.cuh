...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1_impl_vmmq(
    ...
    const uint8_t * __restrict__ m, const half2 & dm4, const float * __restrict__ d8, const float * __restrict__ s8) {
...
#pragma unroll
    for (int i = 0; i < QR4_K; ++i) {
        ...
        if constexpr (saved_sum) {
            // Four lanes share the stored sum of each Q8_1 block.
            sumf_m += s8[i] * (0.25f * m[i]);
        } else {
...
static __device__ __forceinline__ float vec_dot_q5_K_q8_1_impl_vmmq(
    ...
    const uint8_t * __restrict__ m, const half2 & dm5, const float * __restrict__ d8, const float * __restrict__ s8) {
...
static __device__ __forceinline__ float vec_dot_q4_K_q8_1(
    ...
    return vec_dot_q4_K_q8_1_impl_vmmq<saved_sum>(v, u, sc, m, bq4_K->dm, d8, s8);
}
...
#pragma unroll
    ...
    return vec_dot_q5_K_q8_1_impl_vmmq<saved_sum>(vl, vh, u, sc, m, bq5_K->dm, d8, s8);
}
